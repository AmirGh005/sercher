...
# محاسبه نوسانات
data['Volatility'] = data['Close'].pct_change().abs()

# تعیین آستانه برای شناسایی نوسانات غیرعادی (مثلاً میانگین + 2 انحراف معیار)
vol_threshold = data['Volatility'].mean() + 2 * data['Volatility'].std()

# شناسایی نقاط با نوسانات غیرعادی
data['Unusual'] = data['Volatility'] > vol_threshold

# حذف سطرهای دارای نوسانات غیرعادی و مقدارهای NaN (به علت محاسبات اندیکاتورها)
data = data[~data['Unusual']]
...
# ساخت مدل و آموزش اولیه
model = build_model()

early_stopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)
model.fit(X_train, y_train, batch_size=16, epochs=100, validation_data=(X_test, y_test), callbacks=[early_stopping])
...
# آنالیز وضعیت فعلی برای بررسی نوسان طبق تحلیل تکنیکال
if not data['Unusual'].iloc[-1]:  # اگر آخرین نوسان غیرعادی نباشد
    ...
    for _ in range(tedadRooz):
        ...
        # افزودن پیش‌بینی به داده‌های واقعی
    ...
    for i in range(1, tedadRooz+1):
        print(f'{i} forcast:{predicted_prices[i-1:i]}')
else:
    print('موقعیت فعلی طبق تحلیل تکنیکال نوسان نمی‌کند و پیش‌بینی انجام نمی‌شود.')
...
# رسم نمودار ترکیبی 15 روز آخر واقعی + 5 روز پیش‌بینی‌شده در صورت پیش‌بینی
if not data['Unusual'].iloc[-1]:
