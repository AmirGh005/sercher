...
predicted_prices = []
tedadRooz = 5 
epsilon = 0.005  # نویز تصادفی کوچک برای جلوگیری از روند ثابت
...
for _ in range(tedadRooz):
    ...
    # افزودن پیش‌بینی به داده‌ها به عنوان داده واقعی به همراه نویز تصادفی کوچک
    new_row = np.zeros((1, 3))
    new_row[0, 0] = prediction + np.random.normal(0, epsilon)  # افزودن نویز به پیش‌بینی
    new_row[0, 1] = new_row[0, 0]  # SMA = قیمت بسته‌شده فرضی
    new_row[0, 2] = new_row[0, 0]  # EMA = قیمت بسته‌شده فرضی
